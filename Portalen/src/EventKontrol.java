...
public class EventKontrol {
	...
	public void listOfEvents() {
		if (currentBruger.getEvents().size() == 0) {
			...
			for (Event event : currentBruger.getEvents()) {
				System.out.println("Genren er: " + event.getGenre() + 
									" Kunstneren er: " + event.getKunstner() + 
									" Afholdes i lokale: " + event.getLokale() +
									" Finder sted d.; " + event.getDato());
	...
	public void deleteEvent() {
		for (Event event : currentBruger.getEvents()) {
			System.out.println(currentBruger.getEvents().indexOf(event) + ": " + event.getGenre() + ", " + event.getKunstner() + ", "
					+ event.getLokale() + ", " + event.getDato());
